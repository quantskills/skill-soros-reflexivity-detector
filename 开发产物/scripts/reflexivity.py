...
def vol_divergence_series(panel: pd.DataFrame, win: int) -> pd.Series:
    ...
    for c in ("volume", "amount", "turnover"):
        if c in panel:
            vol = panel[c].astype(float)
            break
    if vol is None:
        return pd.Series(0.0, index=idx, name="VolDiv")
    # 价格离滚动高点的接近度（1=正在创新高）
    roll_hi = close.rolling(win, min_periods=5).max()
    near_high = (close / roll_hi.replace(0, np.nan)).clip(0, 1).fillna(0.0)
    # 量能趋势：20 日均量相对 win 内均量的比值（<1 = 缩量）
    v_ma = vol.rolling(20, min_periods=5).mean()
    v_base = vol.rolling(win, min_periods=5).mean()
    v_ratio = (v_ma / v_base.replace(0, np.nan)).fillna(1.0)
    # 背离 = 价近高点(≥0.95) 且 量萎缩(<1)；强度按二者共同程度线性放大
    price_part = ((near_high - 0.95) / 0.05).clip(0, 1)      # 0.95→0, 1.0→1
    vol_part = ((1.0 - v_ratio) / 0.4).clip(0, 1)             # 缩量 40% → 满分
    return (price_part * vol_part * 100.0).fillna(0.0).rename("VolDiv")


def drawdown_series(panel: pd.DataFrame, win: int) -> pd.Series:
    """DD：距 `win` 内滚动高点的回撤比例（0..1）。破裂判定用。"""
    close = panel["close_adj"].astype(float)
    roll_hi = close.rolling(win, min_periods=5).max()
    return (1.0 - close / roll_hi.replace(0, np.nan)).fillna(0.0).clip(0, 1).rename("DD")
...
def compute_series(panel: pd.DataFrame, events: Optional[list[dict]] = None,
    ...
    # —— 破裂判定四件套：回撤 / 顶背离(曾出现) / 破裂前曾走出趋势 / 燃料退潮 ——
    DD = drawdown_series(panel, pw)
    VolDiv = vol_divergence_series(panel, pw)
    # 顶背离是"顶部"信号：跌下来之后当日 VolDiv 自然归零，所以取近 60 日峰值
    # ——语义是"顶部出现过量价背离，随后跌破"，正是清单要的反转信号。
    VolDivPeak = VolDiv.rolling(60, min_periods=1).max().fillna(0.0).rename("VolDivPeak")
    # 破裂前曾走出明确趋势（阴跌一路的票不该被判"破裂"）。
    # 用 P 峰值而非 P_pctl 峰值：P_pctl 在 pct_win=250 时需 310 日历史才非 NaN，短历史会静默失效。
    PeakP = P.rolling(pw, min_periods=5).max().fillna(-9.9).rename("PeakP")
    # 燃料退潮：FB_long 相对 20 日前的下滑幅度
    FBL_drop = (FBL.shift(20) - FBL).fillna(0.0).rename("FBL_drop")

    df = pd.concat([panel.get("close_adj"), P, P_acc, Fdf, FBL, FBN, CogF, ParF,
                    Sync, GAP, GAP_pctl, P_pctl, Attn, Money, FastLoop,
                    DD, VolDiv, VolDivPeak, PeakP, FBL_drop], axis=1)
...
def _raw_stage(r: pd.Series, cfg: dict) -> str:
    ...
    DD = _safe(r.get("DD"))
    VolDivPeak = _safe(r.get("VolDivPeak"))
    PeakP = _safe(r.get("PeakP"), -9.9)
    FBL_drop = _safe(r.get("FBL_drop"))
    # 负反身性侧：下跌趋势已确立（跌→抛售→再跌）
    if P <= cfg["neg_p"] and Fs <= 0:
        return "S5"
    # 破裂：曾走出明确趋势 + 自高点显著回撤 + (顶部量价背离 或 燃料退潮 或 基本面转弱)
    # ——回路断了但还没跌成负反身性趋势，正是"该走"的窗口；
    #   对应清单「反转信号 = 斜率背离 + 交易量顶背离」。
    if (PeakP >= cfg["p_s2"] and DD >= cfg["break_dd"]
            and (VolDivPeak >= cfg["voldiv_break"]
                 or FBL_drop >= cfg["fuel_drop_break"] or Fs < 0)):
        return "S4"
    ...
    for i in range(n):
        ...
        if i >= confirm - 1:
            ...
            if len(set(window)) == 1 and window[0] != cur:
                nxt = window[0]
                # 破裂是"从回路里掉出来"的转移，不是单日独立判据：
                # 只有先前确实处在反身性阶段(S1/S2/S3)才谈得上"回路破裂"；
                # 从未入过回路的票即便深度回撤也只是普通下跌 → 落 S0，不冒认 S4。
                if nxt == "S4" and cur not in ("S1", "S2", "S3"):
                    nxt = "S0"
