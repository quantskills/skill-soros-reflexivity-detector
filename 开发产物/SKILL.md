...
- 解决问题：把"价格自己制造行情"的**强度与阶段**量化出来——回答交易上三个要命问题：
  ...
  2. 循环转到**哪一圈**了（决定仓位纪律）？
  3. **燃料**（新钱）还在进吗、**油箱裂缝**（质押/解禁/大宗）在漏吗（何时下车、出事跌多深）？
...
| S2 加速 | 大众追涨、燃料足、在加速（含 **考验 S2T**：回调洗盘扛过则 conviction+1） | **add** |
| S3 狂热→暮色 | 分位极高、基本面跟不上；**"利好不涨"(CogF 衰减)= 暮色第一信号** | **reduce** |
| S4 破裂 | 自高点回撤 ≥ `break_dd`(15%) + （**量价顶背离** 或 燃料退潮 或 基本面转弱） | exit |
...
| `fuel_drop` | 燃料 20 日下滑幅度 | 门槛 `fuel_drop_break = 20`（融资/游资明显退潮） |
...
| FB_long / FB_neg | 燃料表（新钱多少）/ 油箱裂缝（高质押、大解禁、折价出货） |
